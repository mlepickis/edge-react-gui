...
import { EdgeText, HeaderText, Paragraph } from '../themed/EdgeText'
import { SimpleTextInput } from '../themed/SimpleTextInput'
import { Radio } from '../themed/ThemedButtons'
import { CardUi4 } from '../ui4/CardUi4'
import { ButtonsModal } from './ButtonsModal'

export type SurveyModalResult = 'submit' | 'dismiss'
...
export const SurveyModal = (props: { bridge: AirshipBridge<SurveyModalResult | undefined> }) => {
  const { bridge } = props
  ...
  const shuffledOptions = SURVEY_OPTS.slice(0, -1).sort(() => Math.random() - 0.5)
  const randomizedSurveyOpts = [...shuffledOptions, SURVEY_OPTS[SURVEY_OPTS.length - 1]]
  ...
  const handleSubmitPress = useHandler(async () => {
    if (selectedIndex == null) return false // Shouldn't happen, button is disabled if no selection
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Paragraph center>{lstrings.survey_discover_subtitle}</Paragraph>
        <CardUi4>
